...
#below line should avoid permission denied error
CONTAINER_CMD_RUN="$CONTAINER_RUN_UTILITY run -it --rm --name $CONTAINER_NAME -p $CONTAINER_PORT:8082 -v $CONFIG_DIR:$CONFIG_DIR_MOUNT -v $UPLOAD_DIR:$UPLOAD_DIR_MOUNT -e CONFIG_DIR=$CONFIG_DIR_MOUNT -e UPLOAD_DIR=$UPLOAD_DIR_MOUNT -e EXPOSED_PORT=$CONTAINER_PORT --privileged $CONTAINER_IMAGE_NAME:$CONTAINER_IMAGE_VERSION"
